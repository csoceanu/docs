...
**Configure the Retention Period**: Ensure the retentionDays field in your ReleasePlan is set to a reasonable value that reflects your team's release cadence. A shorter retention period ensures Release CRs expire more quickly, freeing up their associated Snapshots for garbage collection.
...
* `Release` CRs have their own time-based expiration, defined by the `retentionDays` field in the associated xref:releasing:create-release-plan.adoc[`ReleasePlan`] (default: **14 days**). This *is* configurable per `ReleasePlan`.

...
* *Create a `Release` object:* The development team creates a xref:creating-release.adoc[`Release`] object to reference a specific Snapshot and ReleasePlan. It indicates the users' intent to operate on the Snapshot with the matched ReleasePlanAdmission.

* *Configure `ReleaseServiceConfig`:* The Managed Environment team can configure a xref:configuring-releaseserviceconfig.adoc[`ReleaseServiceConfig`] to define security policies, advisory repository, notification settings, and resource limits for release operations.
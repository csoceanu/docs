...
spec:
 ...
 displayName: 'SRE Production Release' <.>
...
<.> The display name of the ReleasePlan.
<.> The release strategy to use for this ReleasePlan. Valid values are `rolling`, `blue-green`, `canary`, or `immediate`.
...
<.> Optional: The KCP workspace to which the system deploys the application. This workspace is created by the Managed environment team (for example, your organization's SRE team).

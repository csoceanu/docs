...
spec:
  ...
  gracePeriodSeconds: <grace-period> <.>
...
<.> The target deployment environment (e.g., `development`, `staging`, `production`, `testing`).
<.> The grace period in seconds for release shutdown operations.
<.> Set to `true` to enable automatic rollback for this release, `false` otherwise.

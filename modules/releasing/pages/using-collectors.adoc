...
spec:
  ...
  pipelineRef:
    ...
    resolver: jira <.>
    timeout: "60s" <.>
  serviceAccount: collector-service <.>
...
<.> Optional: Timeout for the pipeline. Defaults to 0 (no timeout).
<.> ServiceAccount to use in the Pipeline to gain elevated privileges.
